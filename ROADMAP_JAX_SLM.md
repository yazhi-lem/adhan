...
3. Instruct variant produces grammatical Tamil on a 20-prompt smoke set.
...
6. Model card + license + one-command inference published.
...
## 6. Repository map (new)
```
src/adhan_slm/
├── tokenizer/   swaram_tokenizer.py  (+ tests)   # akshara + morpheme tokens
├── model/       transformer.py                    # Flax SLM (nano/tiny/mini)
├── training/    train_jax.py, mlflow_utils.py     # JAX loop + MLflow tracking
├── eval/        (perplexity, morphology probes)
