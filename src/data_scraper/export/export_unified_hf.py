...
def stratified_selection(records: List[Dict], modern_ratio: float = 0.60) -> List[Dict]:
    ...
    modern_selected = sorted(modern_pool, key=lambda x: x.get("_weight", 0), reverse=True)[
        :modern_target
    ]
    ...
    classical_selected = sorted(classical_pool, key=lambda x: x.get("_weight", 0), reverse=True)[
        :classical_target
    ]

    selected = modern_selected + classical_selected
    print(
        f"Selected {len(selected)} records ({len(modern_selected)} modern + {len(classical_selected)} classical)"
    )
...
def write_readme(
    train: List[Dict], val: List[Dict], test: List[Dict], output_dir: Path, strategy: str
):
...
def main():
    ...
    parser.add_argument(
        "--input", type=str, required=True, help="Input JSONL file (e.g., all_sentences.jsonl)"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="data/pre_training/tamil_texts/hf",
