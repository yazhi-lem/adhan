...
COLLOQUIAL_PAT = re.compile(
    r"\b(ன்னு|ச்சு|வச்சேன்|வச்சி|போயிட்ட[ோ]ன்|போயிட்றேன்|ஏங்க|யானா|யாரு|என்ன|எந்த|டா|யே|யா|யோ|ஹெய்|வா|வாங்க|சார்|சின்ன)\b",
    re.I,
)
...
def collect_candidates(
    scraper: TamilCorpusScraper, sources: List[str], max_articles: int = 1000
) -> List[Dict]:
            ...
            for p in list(mdir.rglob("*.txt")) + list(mdir.rglob("*.jsonl")):
                ...
                if p.suffix == ".jsonl":
                    for line in txt.splitlines():
                        ...
                        if t and len(t) > 40:
                            candidates.append(
                                {"text": t, "source": "local_manifest", "url": obj.get("url")}
                            )
                else:
...
def main():
    ...
    print(
        f"Collecting from sources: {sources} (target n={args.n}, colloq_ratio={args.colloq_ratio})"
    )
    ...
    with jsonl_out.open("w", encoding="utf-8") as jf, txt_out.open("w", encoding="utf-8") as tf:
        for r in selected:
            obj = {
                ...
                "tamil_fraction": r.get("tamil_fraction", 0.0),
                "is_colloquial": bool(r.get("is_colloquial")),
            }
