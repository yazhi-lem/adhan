...
def collect_articles(
    scraper: TamilCorpusScraper, category: str = "Tamil_language", max_articles: int = 500
) -> List[Dict]:
...
def main():
    ...
    parser.add_argument(
        "--include",
        type=str,
        default="wikipedia,literature,news,social,local",
        help="Comma separated sources to include: wikipedia,literature,news,social,local",
    )
    parser.add_argument(
        "--to_hf", action="store_true", help="Also export HuggingFace-style train/val/test JSONL"
    )
    ...
    if not articles:
        logger.info(
            "No category articles found via API; falling back to scraper.scrape_wikipedia()"
        )
    ...
    if not sentences and "local" in include:
        logger.info(
            "No Wikipedia sentences found; falling back to local corpus (pdf_texts / manifests / raw_html)"
        )
    ...
    logger.info(
        f"Extracted {len(sentences)} candidate sentences from {len(articles)} articles (plus local fallback)"
    )
        ...
        with out_file.open("w", encoding="utf-8") as fh:
            for s in top_n:
                obj = {
                    ...
                    "tamil_fraction": s["tamil_fraction"],
                }
